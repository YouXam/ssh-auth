ssh-auth: ssh-auth-server src/main.go src/crypto.go src/database.go src/publickeys.go src/server.go src/ssh.go src/users.go
	cp -r bin src
	cd src && CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o ../ssh-auth-darwin-arm64
	cd src && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o ../ssh-auth-linux-amd64
